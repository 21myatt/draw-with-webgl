...
import type { ShaderModeId } from './webgl/shaders/registry'
...
export default function App() {
  ...
  const actionsRef = useRef<{ clear: () => void; plant: () => void; setMode: (mode: ShaderModeId) => void; setPalette: (index: number) => void; setSpecies: (index: number) => void; setWind: (x: number, y: number) => void }>({ clear: () => undefined, plant: () => undefined, setMode: () => undefined, setPalette: () => undefined, setSpecies: () => undefined, setWind: () => undefined })
  const seasons = ['spring', 'summer', 'autumn', 'winter']
  const modes: ShaderModeId[] = ['flowers', 'stars', 'smoke', 'crystals', 'halos']
  ...
  useEffect(() => {
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      if (event.key.toLowerCase() === 'm') setModeIndex((current) => { const next = (current + 1) % 5; actionsRef.current.setMode(modes[next]); return next })
  ...
  return <main className="flower-app">
    ...
    <GlassOverlay started={started} drawing={drawing} mode={modes[modeIndex]} season={seasons[seasonIndex]} onModeChange={() => { const next = (modeIndex + 1) % 5; setModeIndex(next); actionsRef.current.setMode(modes[next]) }} onSeasonChange={() => { const next = (seasonIndex + 1) % 4; setSeasonIndex(next); actionsRef.current.setPalette(next) }} onClear={() => clearRef.current()} />
  </main>
}
